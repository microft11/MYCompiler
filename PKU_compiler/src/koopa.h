...
// 防止IDE报错，从https://github.com/pku-minic/koopa/blob/master/crates/libkoopa/include/koopa.h 上copy而来
#include <stddef.h>
#include <stdint.h>
...
///
/// 从给定的文件中解析文本形式的Koopa IR程序。
/// 如果没有发生错误，将更新`program`。
///
...
///
/// 从给定的文件描述符（UNIX）或句柄（Windows）中解析文本形式的Koopa IR程序。
/// 如果没有发生错误，将更新`program`。
///
/// 返回错误代码。
...
///
/// 生成以null结尾的文本形式的Koopa IR程序字符串到给定的缓冲区。
/// 如果给定的缓冲区为null，则更新`len`为生成字符串的长度（不包括null终止符）
///
/// Returns the error code.
///
